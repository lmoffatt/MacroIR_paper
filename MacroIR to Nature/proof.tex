...
Here, we present the second new algorithm, MacroIR (Macroscopic Interval Recursive), which we actually used in our analysis. MacroIR performs a Bayesian analysis of an averaged Markovian process over successive time intervals. The formulation is made in terms of the response we typically obtain from a macropatch preparation, where high-quality kinetic data of the current flowing through the patch can be recorded with relatively low noise. However, the same approach can be adapted to other systems, such as responses measured with FRET, molecular motor ensembles, or potentially in diffusion analysis.

The theoretical foundation was laid down in the MicroIR algorithm, where we constructed a meta-state that includes both the initial state at the start of the interval and the state at the end of the interval. By leveraging this concept and through algebraic simplification, we reduced the complexity of the resulting equations from \( k^4 \) to \( k^3 \).

\subsubsection{Meta-State Definitions}

Let's define the meta-state: the idea is to describe both the initial ensemble state and the final ensemble state during the measurement interval \( [0, t] \). For clarity, we use the convention that \( 0 \) marks the start of the interval and \( t \) marks its end.
...
The average current of the meta-state \( (i_0 \rightarrow i_t) \), denoted as \( ({\gamma}_{0 \rightarrow t})_{(i_0 \rightarrow i_t)} \), is defined as the average current conditional on starting at state \( i_0 \) and ending at state \( i_t \):
...
Similarly, the variance of the average current of the meta-state \( (i_0 \rightarrow i_t) \) is given by:
...
Applying Eq.~\ref{eq:macro_likelihood} to the meta-state, we obtain an expression for the likelihood:
...
where, using Eq.~\ref{eq:macro_predicted_y},
...
Now, let us expand the term \( \mathbf{\mu}^{\text{prior}}_{0 \rightarrow t} \cdot \mathbf{\gamma}_{0 \rightarrow t} \):
...
We update the posterior mean as:

\begin{equation}
	\mathbf{\mu}^{\mathrm{post}}_{0, t} = \mathbf{\mu}^{\mathrm{prior}}_{0, t} + 
	\frac{y^{\mathrm{obs}}_{0, t} - y^{\mathrm{fit}}_{0, t}}{{\sigma^2}^{\mathrm{fit}}_{0, t}} 
	\cdot \mathbf{\gamma}^{\mathrm{T}}_{0, t} \cdot \mathbf{\Sigma}^{\mathrm{prior}}_{0, t}
	\label{eq:posterior_mean_update}
\end{equation}
...
\subsection{MacroIR, Bayesian Framework for Interval Averaged Measurements of Channel Ensembles: The MacroIR Algorithm}

Here we present the second new algorithm, MacroIR for Macroscopic Interval Recursive, one we actually used. It performs Bayesian analysis of an averged markovian process over a succesive time intervals. 
The formulation is made in terms of the response we tipically obtain from a macropatch preparation where high quality kinetic data of the current that goes trough the patch is obtain at relatively low noise, but the same approach can be adapted to other systems like responses measured with FRET or ensemble of molecular motors and possibly in diffusion analysis. 
The theoretical fundation was already laid down in the MicroIR algorithm where we construct a meta-state that includes the initial state at the start of the interval and the state at the end of the interval. Then what we do is to apply Eq.~\ref{eq:macro_predicted_y},\ref{eq:macro_likelihood},\ref{eq:macro_sigma_pred}, \ref{eq:macro_mean_posterior} and \ref{eq:macro_cov_posterior} of MacroR. Some algebra is done and magically the complexity of the resulting equations is diminished from  $k^4$ to $k^3$. 

So, lets define this meta-state: the idea is that the description of the state considers now both the initial ensemble state and the final ensemble state during the measuring interval $[0,t]$. The equations look nicer by using the convention that 0 is the start of the interval and t is the end of it.  
So, $(\mu^{prior}_{0, t})_{(i_0, i_{t})}$ the mean probability for starting at state $i_0$ and ending at state $i_t$ is obtained using results from conditional expectation:

\begin{equation}
	(\mu^{prior}_{0 \rightarrow  t})_{(i_0 \rightarrow i_{t})} = (\mu^{prior})_{i_0}  \cdot P_{i_0 \rightarrow i_{t}}(t)
	\label{eq:meta_mean_prior}
\end{equation}

In the same way, $(\Sigma^{prior}_{0,t})_{(i_0 \rightarrow i_{t})(j_0 \rightarrow j_{t})}$ the covariance between the join probability of starting at state $i_0$ and ending at state $i_t$ and the join probability of starting at state $j_0$ and ending at state $j_t$ is also calculated using conditional expectations: 

\begin{multline}
	(\Sigma^{prior}_{0 \rightarrow t})_{(i_0 \rightarrow i_{t})(j_0 \rightarrow j_{t})} =
	P_{i_0 \rightarrow i_{t}} \left((\Sigma^{prior}_{0})_{i_0 ,j_0} - \delta_{i_0, j_0} \cdot (\mu^{prior}_0)_{i_0} \right)  \cdot P_{j_0 \rightarrow j_{t}} \\
	+ \delta_{i_0, j_0} \cdot \delta_{i_{t}, j_{t}} \cdot (\mu^{prior}_0)_{i_0}\cdot P_{i_0 \rightarrow i_{t}} 
	\label{eq:meta_covariance_prior}
\end{multline}
where $\delta_{i, j}$ is $0$ if $i \neq j$ and $1$ if $i=j$. 

Now,$({\overline \gamma}_{0 \rightarrow t })_{(i_0 \rightarrow  i_{t})}$, the average current of the meta state $(i_0 \rightarrow i_t)$ is just the average current conditional on starting at state $i_0$ and ending at state $i_t$: 

\begin{equation}
	({\gamma}_{0 \rightarrow t })_{(i_0 \rightarrow i_{t})} = {\overline \Gamma}_{i_0 \rightarrow i_{t}}
	\label{eq:meta_gamma_prior}
\end{equation}
and similarly for the variance of the average current of the meta state $(i_0 \rightarrow i_t)$

\begin{equation}
	(\sigma^2{\gamma}_{0 \rightarrow t})_{(i_0 \rightarrow  i_{t})} = \sigma^2{\overline \Gamma}_{i_0 \rightarrow i_{t}}
	\label{eq:meta_sigma_gamma_prior}
\end{equation}



Lets now apply Eq. \ref{eq:macro_likelihood} to this meta-state we obtain an expression of the likelihood
\begin{equation}
	\mathcal{L}_{0 \rightarrow  t}= \mathcal{N}(\overline{y}^{obs}_{0 \rightarrow t}-\overline{y}^{pred}_{0 \rightarrow  t}, {\sigma^{2}}_{\overline{y}^{pred}_{0 \rightarrow  t}})
	\label{eq:meta_macro_likelihood}
\end{equation}
where , applying Eq. \ref{eq:macro_predicted_y} 
\begin{equation}
	\overline{y}^{pred}_{0,t} = N_{ch} \cdot \mathbf \mu^{prior}_{0,t} \cdot \gamma_{0,t}
	\label{eq:meta_macro_predicted_y}
\end{equation}

now, 
\begin{multline}
	\mathbf \mu^{prior}_{0 \rightarrow t} \cdot \mathbf \gamma_{0 \rightarrow t} =
	\sum_{i_0, i_t} {(\mu^{prior}_{t})_{i_t}  \cdot P_{i_0 \rightarrow i_{t}} \cdot ({\overline \Gamma_{0 \rightarrow t}})_{i_0 \rightarrow i_{t}}}= \mathbf \mu^{prior}_0 \cdot  \overline {\mathbf \gamma}_{0}
\end{multline}

where the average current is obtained by marginalizing over the end state 
\begin{equation}
    (\overline {\mathbf \gamma}_{0})_i =\sum_j {\overline \Gamma}_{i \rightarrow j}
\end{equation}


In this way, we obtain an expression for the predicted y that does not depend on the meta state but directly on the prior mean state vector and the averaged current conditional on the start state:   

\begin{equation}
	\overline{y}^{pred}_{0,t} = N_{ch} \cdot \mathbf \mu^{prior}_{0} \cdot \overline{\gamma}_{0}
	\label{eq:macro_interval_predicted_y}
\end{equation}


 
 For calculating the variance of the prediction we add an extra term to Eq. \ref{eq:macro_sigma_pred}: the contribution of ${\sigma^2 \mathbf  \Gamma}_{0 \rightarrow t}$,the variance of the averaged current, i.e., the open channel noise, 
\begin{equation}
	{\sigma^2}_{\overline{y}^{pred}_{0 \rightarrow t}}
	= \epsilon^2_{0 \rightarrow t} +N_{ch} \cdot {\mathbf \gamma}^{\mathrm{T}}_{0 \rightarrow t} \cdot \mathbf \Sigma^{prior}_{0 \rightarrow t} \cdot \mathbf \gamma_{0 \rightarrow t}
	+ N_{ch} \cdot \mathbf \mu^{prior}_{0 \rightarrow t} \cdot {\sigma^2 \mathbf  \Gamma}_{0 \rightarrow t}
	\label{eq:meta_macro_sigma_pred}
\end{equation}

lets try to find simpler expressions for the meta-state matrix multiplications. 
In the case of 

\begin{multline}
	{\mathbf \gamma}^{\mathrm{T}}_{0,t} \cdot \mathbf \Sigma^{prior}_{0,t} \cdot \mathbf \gamma_{0,t}= \\
...
\begin{multline}
	{\mathbf \gamma}^{\mathrm{T}}_{0,t} \cdot \mathbf \Sigma^{prior}_{0,t} \cdot \mathbf \gamma_{0,t}= \\
	{\overline {\mathbf \gamma}_{0,t}}^\mathrm{T} \cdot 
	(\mathbf \Sigma^{prior}_{t} -\mathrm{diag}( \mathbf \mu^{prior}_t))\cdot  
	{\overline {\mathbf \gamma}_{0,t}}+ 
...
\begin{equation}
	( \mu^{prior}_{t+ \Delta t})_{i_{t+\Delta t}} = \sum_{i_t} (\mu^{post}_{0,t})_{(i_t, i_{t+\Delta t)}}
\end{equation}
\begin{equation}
\end{equation}
