...
\begin{equation}
	\boldsymbol{p}(t) = \boldsymbol{p}(0) \cdot \exp(\boldsymbol{Q} \cdot t),
...
\begin{equation}
	y_{\text{obs}}(t) = \boldsymbol{p}(t) \cdot \boldsymbol{\gamma}.
	\label{eq:eq3}
\end{equation}
...
\begin{equation}
	L = P(y^{\text{obs}}) = \sum_i \mathcal{N}(p_i \cdot \gamma_i, \epsilon^2),
	\label{eq:likelihood}
\end{equation}

where \( \boldsymbol{\gamma} \) is the current vector, with each component \( \gamma_i \) representing the current generated by state \( i \).
...
Now, suppose a second measurement is taken after a time interval \( t \). The prior probability distribution at time \( t \), denoted \( \boldsymbol{p}^{\text{prior}}(t) \), is obtained from the previous posterior distribution and the transition matrix \( \boldsymbol{P} \):
...
where \( \mathcal{N}(\mu, \sigma^2) \) denotes the normal distribution with mean \( \mu \) and variance \( \sigma^2 \), and \( \overline{\Gamma}_{ij} \) and \( \sigma^2_{\overline{\Gamma}_{ij}} \) represent the average current and variance of the average current for starting at state \( i \) and ending at state \( j \), respectively.
