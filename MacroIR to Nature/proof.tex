...
MacroIR (Macroscopic Interval Recursive) is a Bayesian framework for analyzing time-averaged Markovian processes over successive intervals. The method applies to macropatch preparations, FRET measurements, molecular motor ensembles, or diffusion analysis. Building on the MicroIR algorithm, MacroIR reduces the complexity of likelihood calculations from \( \mathcal{O}(k^4) \) to \( \mathcal{O}(k^3) \) by defining a meta-state incorporating both the initial and final states during the measurement interval \( [0, t] \).

The prior mean probability of starting in state \( i_0 \) and ending in state \( i_t \) is:
...
The prior covariance between two joint states \( (i_0 \rightarrow i_t) \) and \( (j_0 \rightarrow j_t) \) is:
...
The meta-state current is defined as:
...
Finally, the posterior mean update is:
...
The theoretical foundation of MacroIR builds upon the earlier MicroIR algorithm, where we construct a meta-state that incorporates both the initial state at the start of the interval and the state at the end. By applying this concept and simplifying algebraically, we reduce the complexity of the resulting equations from \( \mathcal{O}(k^4) \) to \( \mathcal{O}(k^3) \).
