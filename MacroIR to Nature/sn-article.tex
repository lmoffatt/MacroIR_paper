...
Our kinetic schemes propose two distinct allosteric coupling mechanisms depending on whether modulation involves the subunit to the left or to the right of the ATP binding site. Figure~\ref{fig:rates_SchemeX}a illustrates this concept. To underscore the correspondence between model states and channel structures, we adopted schematic representations from the study that resolved the open-channel structure of a purinergic receptor \cite{abierta_p2x}. That work proposed that ATP binding is accompanied by rotation of both ligand-contacting subunits; here, we extend that view by positing that subunits can rotate independently rather than synchronously, with ATP binding modulating these subtle conformational changes.

Traditional allosteric models treat coupling as a single parameter that modifies equilibrium constants. In contrast, our kinetic framework explicitly models the individual rate constants, breaking the inherent symmetry between binding and rotation (or gating). Each conformational change—binding, rotation, and gating—is governed by its own coupling parameter, which requires two additional parameters to fully describe the kinetic modulation.

Figure~\ref{fig:rates_SchemeX}a depicts a cubic arrangement of three coupled conformational transitions—agonist binding, left subunit rotation, and right subunit rotation. Each vertex represents a distinct receptor state (ranging from unrotated/free/unrotated to rotated/bound/rotated), and the edges are annotated with the corresponding kinetic expressions. In the absence of ATP, both subunits rotate at identical rates; ATP binding, however, breaks this symmetry. (This schematic is simplified, as additional binding sites would introduce further allosteric factors.)

Figures~\ref{fig:rates_SchemeX}b and \ref{fig:rates_SchemeX}c show representative posterior distributions for the rate constants governing transitions involving a single subunit (i.e. \texttt{b\_on} versus \texttt{b\_off}). Binding rate constants segregate into four clusters corresponding to distinct structural states—both subunits resting, left rotated, right rotated, and both rotated—while rotation rate constants cluster according to binding site occupancy. Colored lines indicate the magnitude of the allosteric coupling factors for both \textit{on} and \textit{off} transitions, with the diagonal axes representing log affinity [$\log(b_{\text{off}})-\log(b_{\text{on}})$] or log efficacy [$\log(r_{\text{on}})-\log(r_{\text{off}})$].

A pronounced asymmetry emerges between the left and right couplings. Specifically, the coupling associated with the right subunit (or equivalently, the binding site on the left) is robust, whereas that for the left subunit is weak. Moreover, an asymmetry between binding and rotation is evident: the binding data display a well-separated cluster for the weak coupling, while the corresponding separation in rotation rates is less pronounced. Notably, rotation of the left subunit markedly reduces the binding rate (i.e. $RB_{b_{\text{on}}} \ll 1$), whereas rotation of the right subunit accelerates it ($BR_{b_{\text{on}}} > 1$).

Figure~\ref{fig:rates_SchemeX}d further illustrates the kinetic allosteric coupling by plotting the logarithm of the \textit{on} rate coupling against that of the \textit{off} rate. A positive \textit{on} log factor indicates that the active state of the allosteric partner accelerates the transition, while a negative value indicates deceleration. Because the equilibrium constant is the ratio of the \textit{on} and \textit{off} rates, the overall allosteric effect is determined by the balance of these contributions. For instance, if the mechanism solely stabilized the active state, one would expect a highly positive \textit{on} factor paired with a moderately negative \textit{off} factor; if it destabilized the resting state, the reverse would occur. When both mechanisms are active, the \textit{on} and \textit{off} factors tend to be of similar magnitude but opposite in sign—a pattern observed for $BR_{b_{\text{on}}}$, which robustly stabilizes the bound state while only modestly destabilizing the free state.

Two kinetic couplings deviate from these expectations. In one instance, ATP binding at the left site increases both the \textit{on} and \textit{off} rates, suggesting a catalytic effect that lowers the energetic barrier for rotation while simultaneously stabilizing the rotated state. Conversely, rotation of the left subunit reduces both binding and unbinding rates, leading to only minor changes in equilibrium yet a substantial reduction in the \texttt{b\_on} rate. This implies an altered energetic barrier—possibly due to steric hindrance at the binding site upon rotation. By assuming a preexponential factor of $10^{-6}$~s, we computed the posterior distributions of the corresponding energetic barriers, which illustrate these deduced energy changes.

\section{Structural Basis of Kinetic Rate Modulation}

Our kinetic schemes propose two distinct allosteric coupling mechanisms, depending on whether modulation involves the subunit to the left or to the right of the ATP binding site. Figure~\ref{fig:rates_SchemeX}a illustrates this concept. To emphasize that the model states correspond directly to channel structures, we adopted schematic representations from the study that determined the open-channel structure of a purinergic receptor \cite{abierta_p2x}, which proposed that ATP binding is accompanied by rotation of both ligand-contacting subunits. We extend this view by positing that subunits can rotate independently rather than synchronously, with ATP binding modulating these conformational changes.

Traditional models of allosterism treat allosteric effects as modifications of equilibrium constants via a single coupling parameter. In contrast, our kinetic framework explicitly models the individual rate constants, thereby breaking the inherent symmetry between binding and rotation (or gating). Each conformational change is governed by its own coupling parameter, necessitating two additional parameters to fully describe the kinetic coupling.

Figure~\ref{fig:rates_SchemeX}a depicts a cubic arrangement of three coupled conformational changes—agonist binding, left subunit rotation, and right subunit rotation. Each vertex represents a distinct receptor state (ranging from unrotated/free/unrotated to rotated/bound/rotated), and the edges are annotated with the kinetic expressions governing the transitions. In the absence of ATP, both subunits rotate at identical rates; however, ATP binding breaks this symmetry. (Note that this schematic is simplified, as it does not include the influence of additional binding sites, which would require extra allosteric factors.)

Figures~\ref{fig:rates_SchemeX}b and \ref{fig:rates_SchemeX}c show representative posterior distributions for the kinetic rate constants associated with single-subunit transitions (i.e. \texttt{b\_on} versus \texttt{b\_off}). In Figure~\ref{fig:rates_SchemeX}b, the binding constants segregate into four clusters corresponding to distinct structural states—both subunits resting, left rotated, right rotated, and both rotated. Similarly, the rotation rate constants segregate according to binding site occupancy (both free, left occupied, right occupied, or both occupied). The colored lines indicate the magnitude of the allosteric coupling factors for both the \textit{on} and \textit{off} transitions, while the diagonal axes represent log affinity [$\log(b_{\text{off}})-\log(b_{\text{on}})$] or log efficacy [$\log(r_{\text{on}})-\log(r_{\text{off}})$].

A clear asymmetry emerges between the left and right couplings. Specifically, the coupling associated with the right subunit (or equivalently, the binding site on the left) is strong, while that for the left subunit is weak. Moreover, an asymmetry is observed between binding and rotation: the binding data reveal a well-separated cluster for the weak coupling, whereas the separation is less pronounced in the rotation rates. Notably, rotation of the left subunit markedly reduces the binding rate (i.e. $RB_{b_{\text{on}}} \ll 1$), whereas rotation of the right subunit accelerates it ($BR_{b_{\text{on}}} > 1$).

Figure~\ref{fig:rates_SchemeX}d further illustrates the kinetic allosteric coupling by plotting the logarithm of the \textit{on} rate coupling against that of the \textit{off} rate. A positive \textit{on} log factor indicates that the active state of the allosteric partner accelerates the transition, while a negative value denotes deceleration. Since the equilibrium constant is given by the ratio of the \textit{on} to the \textit{off} rates, the overall allosteric effect is determined by the difference between these contributions. For example, if the mechanism solely stabilizes the active state, one would expect a highly positive \textit{on} factor paired with a moderately negative \textit{off} factor; if it destabilizes the resting state, the reverse would be true. When both mechanisms operate, the \textit{on} and \textit{off} factors tend to be of similar magnitude but opposite in sign—a pattern observed for $BR_{b_{\text{on}}}$, which robustly stabilizes the bound state while only modestly destabilizing the free state.

Interestingly, two kinetic couplings deviate from these expectations. In one instance, ATP binding at the left site increases both the \textit{on} and \textit{off} rates, suggesting a catalytic effect that lowers the energetic barrier for rotation while simultaneously stabilizing the rotated state. Conversely, rotation of the left subunit reduces both the binding and unbinding rates, resulting in only a minor change in equilibrium but a substantial reduction in the \texttt{b\_on} rate. This observation implies an altered energetic barrier, possibly due to steric hindrance at the binding site upon rotation. By assuming a preexponential factor of $10^{-6}$~s, we computed the posterior distributions of the corresponding energetic barriers, which illustrate the deduced changes in energy.
\section{Structural Basis of Kinetic Rate Modulation}
...
Our kinetic schemes propose two distinct allosteric couplings, depending on whether modulation involves the subunit to the left or to the right of the ATP binding site. Figure~3a illustrates this concept. To emphasize the fact that the model states correspond to channel structures, we used the same drawings of the paper that determined the open-channel structure of a purinergic receptor \cite{abierta_p2x} and proposed that ATP binding is accompanied by the rotation of both ligand-contacting subunits. We extend that view by positing that subunits can rotate independently rather than synchronously, with ATP binding modulating these subtle conformational changes.

Traditional models of allosterism treat allosteric effects as modifications of equilibrium constants via a single coupling parameter. In contrast, our kinetic framework explicitly models the rate constants, thereby breaking the inherent symmetry between binding and rotation (or gating): each conformational change (binding, rotation, gating, etc.) is governed by its own coupling parameter. This approach necessitates two additional parameters to fully describe the kinetic coupling.
