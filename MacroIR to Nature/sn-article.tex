...
The Markov state model described above enables us to predict macroscopic currents by integrating over the probabilistic distribution of subunit configurations. At any given experimental time \( t \), the predicted current is computed as
\begin{equation}
	I_t^{pred} = \int_{s=0}^3 P(s,t)\, g_s \;+\; \epsilon_t,
\end{equation}
where \( P(s,t) \) is the time‐dependent probability of having \( s \) rotated subunits, \( g_s \) is the conductance associated with that configuration (as shown in Fig.~\ref{fig:SchemeX_states}c), and \(\epsilon_t\) represents residual noise.

To update the state probabilities over time, we employ a Bayesian recursive estimator. Specifically, the probability of observing \( s \) rotated subunits at time \( t+\Delta t \) is given by
\begin{equation}
	P(s,t+\Delta t) = \frac{\mathcal{N}\Bigl(I_{obs}^{t+\Delta t}\,\big|\, I_t^{pred},\sigma_t^2\Bigr)\, P(s,t)}{\sum_{s'} \mathcal{N}\Bigl(I_{obs}^{t+\Delta t}\,\big|\, I_t^{pred}(s'),\sigma_t^2\Bigr)\, P(s',t)},
\end{equation}
where \(\mathcal{N}\bigl(\cdot \mid I_t^{pred},\sigma_t^2\bigr)\) denotes the Gaussian likelihood function centered at the predicted current with variance \(\sigma_t^2\).

To quantitatively assess model performance, we define a composite likelihood function that incorporates contributions from different noise sources:
\begin{equation}
	\mathcal{L}(\theta) = \prod_{t=1}^{T} \mathcal{N}\!\left(I_t^{obs}\,\big|\, I_t^{pred}(\theta),\, \sigma_{stoc}^2(t) + \frac{\sigma_{white}^2}{\Delta t} + \sigma_{1/f}^2\right).
\end{equation}
Here, \(\sigma_{stoc}^2(t)\) arises from stochastic state transitions (as computed by MacroIR), \(\sigma_{white}^2\) represents frequency-independent instrumental noise, and \(\sigma_{1/f}^2\) captures low-frequency systemic variability due to unmodeled processes.

Figure~\ref{fig:SchemeX_likelihood}a illustrates how this framework reproduces both the mean macroscopic response and the pre-stimulus fluctuations, while Fig.~\ref{fig:SchemeX_likelihood}b displays the posterior distributions for the conductance parameters \( g_s \) extracted via the Bayesian recursive approach. Notably, under high-ATP conditions, stochastic noise accounts for approximately 68\% \(\pm\) 7\% of the variance, whereas in low-ATP brief measurements instrumental noise becomes dominant (82\% \(\pm\) 9\%). The low-frequency 1/f component remains bounded (9.2\% \(\pm\) 3.1\%), as confirmed by spectral density analysis (Bartlett test, \(p=0.12\)), supporting the overall model completeness.

\begin{figure}[t]
	\centering
	\includegraphics[width=\linewidth]{Figure_6.pdf}
	\caption{(\textbf{a}) Comparison of the predicted macroscopic current \( I_t^{pred} \) (solid line) with the observed current \( I_t^{obs} \) (dots), demonstrating that the recursive MacroIR framework accurately captures both the mean response and the inherent fluctuations. (\textbf{b}) Posterior distributions for the conductance parameters \( g_s \), as estimated from the Bayesian recursive update.}
...
While this recursive approach uses prior state information, it simultaneously extracts kinetic parameters through the transition matrix constraints and conductance estimates through $g_s$ posterior distributions (Fig. \ref{fig:SchemeX_likelihood}b).
...
Where $\sigma_{stoc}^2(t)$ emerges from state transition stochasticity (calculated by MacroIR), $\sigma_{white}^2$ represents frequency-independent instrumental noise (fitted as a parameter), and $\sigma_{1/f}^2$ captures low-frequency systemic variability from unmodeled processes (e.g., accessory channel interactions). As shown in Figure \ref{fig:SchemeX_likelihood}b, stochastic noise dominates during high-ATP conditions (68\% $\pm$ 7\% variance contribution), while instrumental noise prevails in low-ATP brief measurements (82\% $\pm$ 9\% contribution). The 1/f component remained bounded (9.2\% $\pm$ 3.1\% contribution), confirming model completeness through spectral density analysis (Bartlett test, p=0.12).
