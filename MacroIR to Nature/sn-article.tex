...
Traditional allosteric models are based on thermodynamic considerations and thus treat coupling as a single parameter that modifies equilibrium constants. When extending this formalism to kinetic models,it employs two parameters—one for each conformational change—to accurately capture the kinetic modulation. 
Our kinetic schemes propose two distinct allosteric coupling mechanisms, depending on whether modulation involves the subunit to the left (BR, Binding-Rotation) or the subunit to the right (RB, Rotation-Binding) of the ATP binding site (see Figure~\ref{fig:rates_SchemeX}a). To underscore the correspondence between model states and channel structures, we adopted schematic representations from the study that resolved the open-channel structure of a purinergic receptor \cite{abierta_p2x}. That work proposed that ATP binding is accompanied by the rotation of both ligand-contacting subunits; here, we extend this view by assuming that the subunits can rotate independently rather than synchronously, with ATP binding modulating these subtle conformational changes. Figure~\ref{fig:rates_SchemeX}a depicts a cubic arrangement of three coupled conformational transitions—agonist binding, left subunit rotation, and right subunit rotation. Each vertex represents a distinct receptor state (ranging from unrotated/free/unrotated to rotated/bound/rotated), and the edges are annotated with the corresponding kinetic expressions. In the absence of ATP, both subunits rotate at identical rates; however, ATP binding breaks this symmetry. (This schematic is simplified, as additional binding sites would introduce further allosteric factors.)
