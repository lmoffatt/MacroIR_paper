...
% Ensure that your bibliography includes the reference for the supplementary document.
...
\begin{multline}
	...
	\overline{\mathbf{\gamma}}_{0}^{\mathrm{T}} \cdot \Bigl( \mathbf{\Sigma}^{\text{prior}}_{0} - \mathrm{diag}(\mathbf{\mu}^{\text{prior}}_0) \Bigr) \cdot \overline{\mathbf{\gamma}}_{0 \rightarrow t} + \mathbf{\mu}^{\text{prior}}_0 \cdot \Bigl( \overline{\mathbf{\Gamma}}_{0 \rightarrow t} \circ \overline{\mathbf{\Gamma}}_{0 \rightarrow t} \circ \mathbf{P} \Bigr) \cdot \mathbf{1},
...
We introduce MacroIR (Macroscopic Interval Recursive), a Bayesian framework for analyzing time-averaged Markovian processes over successive intervals—as encountered in macropatch recordings. MacroIR leverages the meta-state, which represents the joint probability of starting in state \(i_0\) and ending in state \(i_t\) over an interval \([0,t]\), without explicitly expanding the state space.

The meta-state prior mean is defined as:
\begin{equation}
	(\mu^{\text{prior}}_{0 \rightarrow t})_{(i_0 \rightarrow i_t)} = (\mu^{\text{prior}})_{i_0} \, P_{i_0 \rightarrow i_t}(t),
...
Posterior updates are performed without explicitly computing the meta-state. The posterior mean is updated as:
\begin{equation}
	\mathbf{\mu}^{\text{post}}_{0 \rightarrow t} = \mathbf{\mu}^{\text{prior}}_{0 \rightarrow t} + \frac{y^{\text{obs}}_{0 \rightarrow t} - y^{\text{pred}}_{0 \rightarrow t}}{\sigma^2_{\overline{y}^{\text{pred}}_{0 \rightarrow t}}} \cdot \widetilde{\boldsymbol{\gamma}^{\mathrm{T}} \boldsymbol{\Sigma}},
	\label{eq:macro_interval_posterior_mean}
\end{equation}
...
In summary, MacroIR provides a recursive Bayesian framework for interval-averaged measurements, efficiently computing likelihoods and posterior distributions while circumventing the high computational cost associated with explicit meta-state calculations.
\subsubsection{MacroIR: A Bayesian Framework for Interval-Averaged Analysis}

MacroIR extends the MacroR algorithm to analyze time-averaged Markovian processes over intervals, as in macropatch recordings. It treats the meta-state—representing the joint probability of starting in state \(i_0\) and ending in state \(i_t\) over \([0,t]\)—without the computational cost of explicitly expanding the state space.
...
\end{multline}
	...
	The predicted average current over the interval is given by
	\begin{equation}
	\overline{y}^{\text{pred}}_{0,t} = N_{\text{ch}}\, \mathbf{\mu}^{\text{prior}}_{0}\, \overline{\gamma}_{0},
	\label{eq:macro_interval_predicted_y}
\end{equation}
with \((\overline{\gamma}_{0})_i = \sum_j (\overline{\Gamma})_{i\rightarrow j}\), and its variance by
\begin{equation}
\sigma^2_{\overline{y}^{\text{pred}}_{0\rightarrow t}} = \epsilon^2_{0\rightarrow t} + N_{\text{ch}}\, \widetilde{\mathbf{\gamma}^{\mathrm{T}}\mathbf{\Sigma}\mathbf{\gamma}} + N_{\text{ch}}\, \mathbf{\mu}^{\text{prior}}_{0}\, \sigma^2_{\overline{\gamma}_0},
\label{eq:macro_interval_sigma_pred}
	\end{equation}
	with \((\sigma^2_{\overline{\gamma}_{0}})_i = \sum_j (\sigma^2\,\overline{\Gamma})_{i\rightarrow j}\).

Posterior updates are performed without explicit meta-state calculation. The posterior mean is updated as
\begin{equation}
	\mathbf{\mu}^{\text{post}}_{0\rightarrow t} = \mathbf{\mu}^{\text{prior}}_{0\rightarrow t} + \frac{y^{\text{obs}}_{0\rightarrow t}-y^{\text{pred}}_{0\rightarrow t}}{\sigma^2_{\overline{y}^{\text{pred}}_{0\rightarrow t}}}\, \widetilde{\boldsymbol{\gamma}^{\mathrm{T}}\boldsymbol{\Sigma}},
\label{eq:macro_interval_posterior_mean}
\end{equation}
where the effective gain is
\begin{equation}
	\widetilde{\boldsymbol{\gamma}^{\mathrm{T}}\boldsymbol{\Sigma}} = \mathbf{1}_0^{\mathrm{T}}\, \mathbf{\gamma}^{\mathrm{T}}_{0\rightarrow t}\, \mathbf{\Sigma}^{\text{prior}}_{0\rightarrow t}.
	\label{eq:interval_gamma_sigma}
	\end{equation}
The prior for the next interval is then given by
\begin{multline}
\boldsymbol{\Sigma}^{\text{prior}}_{t} = \mathbf{P}^{\mathrm{T}}\Bigl(\boldsymbol{\Sigma}^{\text{prior}}_{0} - \mathrm{diag}(\boldsymbol{\mu}^{\text{prior}}_{0})\Bigr)\mathbf{P} + \mathrm{diag}(\boldsymbol{\mu}^{\text{prior}}_{0}\,\mathbf{P}) \\
	- \frac{N}{\sigma^2_{\overline{y}^{\text{pred}}_{0\rightarrow t}}}\, \widetilde{\boldsymbol{\gamma}^{\mathrm{T}}\boldsymbol{\Sigma}}^{\mathrm{T}}\,\widetilde{\boldsymbol{\gamma}^{\mathrm{T}}\boldsymbol{\Sigma}}.
	\label{eq:prior_covariance_update_0_t}
\end{multline}

In summary, MacroIR provides a recursive Bayesian framework that efficiently computes the likelihood and posterior for interval-averaged measurements, overcoming the computational challenges of explicit meta-state handling.
\subsubsection{MacroIR: A Bayesian Framework for Interval-Averaged Analysis of Channel Ensembles}
