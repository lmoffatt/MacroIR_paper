...
\begin{figure}[t]
	...
	\caption{\textbf{State Occupancies and Functional Currents Across Rotational States.} (a) Equilibrium state occupancies measured 60\,s after stimulus clearance reveal that stochastic fluctuations yield measurable pre-activation in the absence of ligand, with 15.2\% $\pm$ 1.3\% of channels displaying one rotated subunit, 1.1\% $\pm$ 0.2\% showing two rotated subunits, and 0.02\% $\pm$ 0.01\% achieving full rotation (mean $\pm$ SEM, n=7). (b) Temporal evolution of state probabilities during ATP stimulation indicates that high-concentration ATP pulses (10\,mM) predominantly drive activation via sequential (edge) transitions (ligand binding followed by rotation), whereas deactivation involves central states with mixed bound/unbound configurations (Wilcoxon signed-rank test, p<0.001). (c) Distribution of unitary currents across rotational states demonstrates that partial activation is functionally significant; channels with two rotated subunits produce 48\% $\pm$ 5\% of maximal current (n=112), accounting for the baseline current observed under ligand-free conditions.}
...
Our findings demonstrate that ATP binding induces an asymmetric conformational response among receptor subunits.  Rather than triggering a uniform, concerted transition, ATP binding selectively reduces the energetic barrier for rotation in one subunit while leaving the barrier for its neighbor largely unaltered. Moreover, the rotation of this favored subunit subsequently elevates the energetic barrier for ATP binding at an adjacent site, thereby providing a mechanistic basis for negative cooperativity found in other studies \cite{Sattler2020UnravellingTI}.

Importantly, while the activation kinetic profile could, in principle, be achieved without barrier modulation, simply by initiating from higher basal turnover rates, the differential modulation we observed entails an elevation in the energy barrier for resting-state rotation. This increased barrier limits the time the channel spends in a transitional conformation in the absence of agonist, likely reducing the risk of channel inactivation. Conversely, decreasing both the rotation and unrotation rates increase the time constant of unliganded gating noise, thereby rendering it detectable by our analysis. 
